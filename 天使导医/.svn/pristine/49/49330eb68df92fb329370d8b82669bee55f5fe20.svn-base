 ...
 * 
 */
var base_jsfile = ["js/supper/comm/xSelect.js?v=5", "js/supper/comm/xform.js?v=32", "js/supper/comm/xtoolbar.js?v=6", "js/supper/comm/xdatagrid.js?v=17"];
...
function doService(opt, val) {
	...
	if(val.options&&val.options.type){
		base_default_type=val.options.typ;
	}
...
var initPagination = function(val) {
	...
	paginationService=val.service?val.service: "";
	commService=val.commandindex?val.commandindex:"";
	var att_data=val.data?val.data: "";
	paginationLimit=val.limit?val.limit: 10;
	var allPage = totalCount != 0 ? totalCount/paginationLimit:1;
	paginationBackE = val.BackE ? val.BackE :null;
	if(!att_id){
		 return ;
	}  
	if(val.isAjax != null && val.isAjax == "1"){
		var list =[];
		$.supper("doservice", {"service":paginationService, "data":att_data+"&page=1&limit="+paginationLimit, "BackE":function (jsondata) {
			totalCount = jsondata.totalCount?jsondata.totalCount: 0;
			allPage = totalCount != 0 ? totalCount/paginationLimit:1;
	 		...
	 		$("#"+att_id).pagination(allPage, {
				...
				callback: function(page_index, jq){
					page_index ++;
					paginationCurrentPage = page_index;
					$.supper("doservice", {"service":paginationService, "data":att_data + "&limit="+paginationLimit+"&page="+page_index, "BackE":function (jsondata) {
						paginationBackE.call(this,jsondata.items);
					}});
		...
		$("#"+att_id).pagination(allPage, {
			...
			callback: function(page_index, jq){
				...
				var AjaxUrl = "";
				if(att_data != "")
					att_data += "&page="+page_index+"&limit="+paginationLimit;
				else
					att_data += "page="+page_index+"&limit="+paginationLimit;	
				if(paginationService != ""){
					AjaxUrl = $.supper("getServicePath", {"service":paginationService,"data":att_data ,url:att_url});
				}else{
...
var att_data=val.data?val.data: "";
$.supper("doservice", {"service":paginationService, "data":att_data + "&limit="+paginationLimit+"&page="+paginationCurrentPage, "BackE":function (jsondata) {
		paginationBackE.call(this,jsondata.items);
}});
};
...
var index =top.window.layer.open({
    ...
    maxmin: false,
    scrollbar: false,
    content: '<div id="systemAlert" class="ibox float-e-margins"> <div class="ibox-content" id="AlertSBI" > <h2><i class="'+att_icon+'" style="margin-top: 15px;margin-left: 15px;margin-right: 15px;color:#003399;"></i><small>'+ att_msg+'</small></h2></div></div>',
    icon: 1,
